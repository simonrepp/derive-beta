...
const indexArticles = data => {
    const indexed = Array.from(data.articles.values()).map(article => {
        ...
        const regular = [
            article.abstract ? striptags(article.abstract.converted) : '',
            article.authors.map(author => author.name).join(' '),
            article.categories.map(category => category.name).join(' '),
            article.tags.map(tag => tag.name).join(' '),
            article.text ? striptags(article.text.written) : ''
        ].join(' ');
...
const indexPrograms = data => {
    const indexed = Array.from(data.programs.values()).map(program => {
        ...
        const regular = [
            program.abstract ? striptags(program.abstract.converted) : '',
            ...
            program.text ? striptags(program.text.written) : ''
        ].join(' ');
...
module.exports = data => {
    fsExtra.copySync(
        path.join(__dirname, 'search/index.php'),
