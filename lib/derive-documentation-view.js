  ...
  constructor(data) {
    ...
    this.element.addEventListener('click', event => {
        ...
        atom.workspace.open(filePath).then(editor => {
          if(selection) {
            ...
            editor.setSelectedBufferRange(
              [[parsed.from.line, parsed.from.column],
               [parsed.to.line, parsed.to.column]]
            );
          }
        });
      }
      ...
      if(element.dataset.showTab !== undefined) {
        ...
        for(const toggle of tabToggles) {
          if(toggle === element) {
      ...
      this.info.innerHTML = `
        <div class="block" style="display: flex; justify-content: center;">
          ${this.data.errors.length > 0 ? `
            <h2 class="inline-block text-highlight">
      ...
      errs += `
        <div class="inset-panel derive-panel-warning">
          <div class="panel-heading">
            <span class="text-highlight">${msg}</span>
          </div>
  ...
  registerEditor(item) {
    ...
    this.editorSubscription = item.onDidStopChanging(() => {
      try {
        ...
        const { element, range } = enolib.lookup({ column: bufferPosition.column, line: bufferPosition.row }, item.getText());

        if(range === 'value' && element.yieldsField()) {
          const value = element.toField().optionalStringValue();
