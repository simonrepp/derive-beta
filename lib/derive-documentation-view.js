...
class DeriveDocumentationView {
  ...
  constructor(data, emitter) {
    ...
    main.style.setProperty('height', '50px');
    main.innerHTML = `
          ...
          <div class="block" style="display: flex; justify-content: center;">
            ${data.errors.length > 0 ? `
              <h2 class="inline-block">
                <span class="icon icon-flame"></span> ${data.errors.length} kritische Fehler
              </h2>
            `:''}
            ${data.warnings.length > 0 ? `
              <h2 class="inline-block">
                <span class="icon icon-issue-opened"></span> ${data.warnings.length} Fehler
              </h2>
            `:''}
          </div>
        `;
