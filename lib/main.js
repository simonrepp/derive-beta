...
module.exports = {
  ...
  async activate() {
    ...
    const commands = {
      ...
      'derive-beta:plain-reference': () => this.openReference('plain.plain', 'Plain Data Handbuch.plain'),
        ...
        if(path.extname(file.path) === '.plain') {
          ...
          atom.workspace.open(fullPath).then(editor => {
            if(file.ranges && file.ranges.length > 0) {
              editor.setSelectedBufferRanges(file.ranges.map(range =>
                [[range.beginLine - 1, range.beginColumn],
