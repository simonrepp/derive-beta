...
module.exports = {
  ...
  async activate() {
    ...
    const commands = {
      ...
      'derive-beta:list-issues': () => this.listView('issues', 'Ausgaben Auflistung.txt'),
  ...
  deactivate() {
    ...
    if(this.data.serverProcess) { this.data.serverProcess.kill(); }
  ...
  async preview(site) {
    ...
    if(this.data.serverUrl) {
      atom.notifications.addSuccess(
        `**${site}**\n\nSeite erfolgreich generiert - Du kannst sie nun im Browser testen! \n\nTipp: Andere Computer im lokalen Netzwerk können den lokalen Vorschau Server auch abrufen - einfach im Browser zu der URL \`${this.data.serverUrl}\` navigieren.`,
        {
          buttons: [{
      ...
      atom.notifications.addError(
        `**${site}**\n\nDie Seite wurde erfolgreich generiert, es scheint aber kein Hintergrundprozess für den lokalen Server aktiv zu sein, wurde eine Fehlermeldung diesbezüglich angezeigt? Zur Lösung sollte zunächst Atom neu gestartet werden, falls dass nicht hilft kann in zweiter Instanz ein Neustart des Computers helfen.`
      );
    }
  },
  ...
  async refresh() {
    ...
    this.preview('derive.at'); // TODO: Remove at some point (automatically build on refresh)
  },
