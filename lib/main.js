...
const backup = require('./backup.js'),
      ...
      { loadAdventure } = require('../derive-common/util.js'),
...
module.exports = {
  ...
  subscriptions: null,
  async activate() {
    atom.config.set('whitespace.removeTrailingWhitespace', false, { scopeSelector: '.text.adventure' }); // TODO: Keep this scope up to date (!)
    ...
    const register = async changePathSubscription => {
      ...
      for(let directory of projectFolders) {
        ...
        if(fs.existsSync(configPath)) {
          this.data.config = await loadAdventure(directory, 'derive.plain');
  ...
  async registerPlugin() {
    const commands = {
      ...
      'derive-beta:plain-reference': () => this.openReference('plain.plain', 'Adventure Handbuch.plain'),
      ...
      'derive-beta:discard-ten': () => this.discardTen(),
    };
  ...
  async refresh() {
    ...
    console.log(`${this.data.errors.length} errors`);
    console.log(`${this.data.warnings.length} warnings`);

    const affectedFileButtons = files => files.map((file, index, files) => ({
      onDidClick: () => {
        if(path.extname(file.path) === '.plain') {
          const fullPath = path.join(this.data.root, file.path);

          atom.workspace.open(fullPath).then(editor => {
            if(file.selection) {
              editor.setSelectedBufferRange(
                [[file.selection[0][0] - 1, file.selection[0][1]],
                 [file.selection[1][0] - 1, file.selection[1][1]]]
              );
            }
          });
        } else {
      ...
      text: files.length > 1 ? `Datei ${index + 1} öffnen` : 'Datei öffnen'
    }));

    const errorlikeNotification = errorlike => {
      let description, message;

      if(errorlike.files) {
        if(errorlike.files.length > 1) {
          description = `**Betroffene Dateien**:  \n${errorlike.files.map((file, index) => `${file.path} (${index + 1})`).join('  \n')}`;
        } else {
          description = `**Betroffene Datei**:  \n${errorlike.files[0].path}`;
        }
        message = `**${errorlike.files.map(file => path.basename(file.path)).join('  \n')}**\n\n${errorlike.message}`;
      } else {
      ...
      return [
        message,
        {
          buttons: errorlike.files ? affectedFileButtons(errorlike.files) : [],
          dismissable: true,
          detail: errorlike.snippet,
    ...
    if(this.data.errors.length > 0) {
      this.data.errors.forEach(error => atom.notifications.addError(...errorlikeNotification(error)));
    }

    if(this.data.warnings.length > 0) {
      const addWarnings = () => {
        this.data.warnings.forEach(warning => atom.notifications.addWarning(...errorlikeNotification(warning)));
      };

      if(this.data.warnings.length > 300) {
  ...
  async newTemplate(document) {
    ...
    editor.setGrammar(atom.grammars.grammarForScopeName('text.adventure')); // TODO: Keep this scope up to date (!)
  },
