...
const backup = require('./backup.js'),
      ...
      { loadEno } = require('../derive-common/util.js'),
      serve = require('./serve.js'),
      SuggestionProvider = require('./suggestion-provider.js'),
      sync = require('./sync.js'),
      transform = require('../derive-common/transform.js');

// TODO: Depend on language-eno (possibly ditch the customized markdown syntax thing for now?)
...
module.exports = {
    ...
    const register = async changePathSubscription => {
      ...
      for(let directory of projectFolders) {
        ...
        if(fs.existsSync(configPath)) {
          this.data.config = await loadEno(directory, 'derive.eno');
  ...
  async registerPlugin() {
    const commands = {
      ...
      'derive-beta:eno-reference': () => this.openReference('eno.eno', 'Eno Handbuch.eno'),
