...
const { URBANIZE_EDITIONS } = require('../derive-common/loaders/enum.js');
...
class SuggestionProvider {
  ...
  getSuggestions(options) {
    ...
    try {
      ...
      if(element.yieldsField()) {
        ...
        if(key === 'Urbanize')
          return this.urbanizeSuggestions(filter);
  ...
  urbanizeSuggestions(filter) {
    return URBANIZE_EDITIONS.filter(urbanize => urbanize.match(filter))
                            .map(urbanize => ({
                              text: urbanize,
