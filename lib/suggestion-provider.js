const EnoValue = require('enojs/lib/elements/value.js'); // TODO: Expose this differently through eno
const { parse, EnoParseError } = require('enojs');
...
class SuggestionProvider {
  ...
  getSuggestions(options) {
    ...
    try {
      const document = eno.parse(editor.getText(), { locale: 'de' });

      const lookup = document.lookup(bufferPosition.row + 1, bufferPosition.column);

      if(lookup &&
         lookup.zone === 'value' &&
         (lookup.element instanceof EnoValue) &&
