const AdventureValue = require('../adventurejs/lib/elements/value.js');
const { parse, AdventureParseError } = require('../adventurejs/adventure.js');
const { URBANIZE_ENUM } = require('../derive-common/util.js');

class SuggestionProvider {
  constructor(data) {
    this.selector = '.text.adventure';
    this.data = data;
  }
  ...
  getSuggestions(options) {
    ...
    try {
      ...
      const lookup = document.lookup(bufferPosition.row + 1, bufferPosition.column);

      if(lookup &&
         lookup.zone === 'value' &&
         (lookup.element instanceof AdventureValue) &&
         lookup.element.value &&
         lookup.element.value.length > 1) {
        return this.findMatchingSuggestions(lookup.element);
      }
    } catch(err) {
  ...
  findMatchingSuggestions(element) {
    ...
    if(element.name.match(/^(Redaktion|Autoren\/Herausgeber|Verleger|Veranstalter|Teilnehmer)$/)) {
